...
FBSDKShareLinkContent *sharecontent;
FBSDKShareButton *fbsharebutton;
...
- (IBAction)didTouchMusicCycleButton:(id)sender
{
    switch (_musicCycleType) {
        case MusicCycleTypeLoopAll:
        {
            ...
            [self showMiddleHint:@"Shuffle Playback"];
        } break;
        case MusicCycleTypeShuffle:
        {
...
-(void) setupView
{
    UIView *superView = _volumeControl.superview;
    [_volumeControl removeFromSuperview];
    [_volumeControl removeConstraints:self.view.constraints];
    _volumeControl.translatesAutoresizingMaskIntoConstraints = YES;
    _volumeControl.transform = CGAffineTransformMakeRotation(270.0/90*M_PI_2);
    [superView addSubview:_volumeControl];
    
    [_musicSlider removeFromSuperview];
    [_musicSlider removeConstraints:self.view.constraints];
    _musicSlider.translatesAutoresizingMaskIntoConstraints = YES;
    _musicSlider.transform = CGAffineTransformMakeRotation(270.0/90 * M_PI_2);
    [superView addSubview:_musicSlider];
    
    sharecontent = [[FBSDKShareLinkContent alloc] init];
    sharecontent.contentURL = [NSURL URLWithString:@"https://www.facebook.com/31bridge/"];
    fbsharebutton = [[FBSDKShareButton alloc] init];
    fbsharebutton.shareContent = sharecontent;
    ...
    likebutton.center =  CGPointMake(40, 85);
    [self.view addSubview:likebutton];

    CGRect frame = _volumeControl.frame;
    CGRect shareframe = fbsharebutton.frame;
    
    CGFloat xPosition = CGRectGetWidth(self.view.frame) - CGRectGetWidth(frame);
    ...
    if (IS_IPHONE_4_OR_LESS)
    {
        ...
        _musicMenuButton.hidden = true;
        _dismissbutton.hidden = true;
        _seconddismissButton.hidden = true;
        _iphonedismissbutton.hidden = false;
        shareframe.origin = CGPointMake(ceil(xPosition) - 45, 40);
        fbsharebutton.frame = shareframe;
        
        CGFloat xPosition = CGRectGetWidth(superView.frame) - CGRectGetWidth(frame);
        CGRect sliderFrame = CGRectMake(0, 40, 50, 150);
    ...
    else if (IS_IPHONE_6)
    {
        ...
        _volumeControl.frame = CGRectMake (ceil(xPosition + 32), 90, 50, 230);
        _musicSlider.frame = sliderFrame;
